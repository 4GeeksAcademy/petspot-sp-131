...
def seed_chats():
    ...
    try:
        ...
        if not users or not places:
            return jsonify({"status": "error", "msg": "No users or places found. Run /api/seed first."}), 400

        chats_added = 0
        now = datetime.utcnow()

        # Create conversations between ~30 user-place pairs
        pairs = [(random.choice(users), random.choice(places)) for _ in range(30)]
        seen = set()
        for u, p in pairs:
            key = (u.id, p.id)
            if key in seen:
            ...
            # Each conversation: 3-6 messages back and forth
            n_exchanges = random.randint(3, 6)
            msg_time = now - timedelta(days=random.randint(1, 60), hours=random.randint(0, 12))
