...
from api.models import db, User, Place, EstablishmentType, AdminUser, Review, City, Reservation, ReservationStatus
...
def get_places():
    places = db.session.execute(select(Place).order_by(
        Place.id.desc())).scalars().all() or None
    if places is None:
...
def get_cities():
    cities = db.session.execute(select(City).order_by(
        City.city.asc())).scalars().all() or None
    if cities is None:
